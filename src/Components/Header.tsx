...
const Input = styled(motion.input)`
    transform-origin:right center; //애니메이션이 시작되는 위치
    position:absolute;
    left:-180px;
`;
...
function Header(){
    ...
    //component에 props로 animate를 주는거에 비해
    //함수로 쓰는 것은 애니메이션을 동시에 여러개 발생시킬 때 유용하다.
    const toggleSearch = ()=>{
        if(searchOpen){
    ...
    const onValid =(data:IForm)=>{
        window.location.replace(`${process.env.PUBLIC_URL}/search?keyword=${data.keyword}`);
        ...
        >
            ...
            <Col>
                <Search onSubmit={handleSubmit(onValid)}>
                    ...
                    <Input 
                        {...register("keyword",{required:false,minLength:1})}
