...
function TvPopular(){
    ...
    useEffect(()=>{
        loadingPopular?null:console.log(dataPopular,"tvpopular");
    },[loadingPopular])
    ...
    const increaseIndex=()=>{
        if(dataPopular){
            if(leaving) return;
            setLeaving(true);
            const totalMovies = dataPopular?.results.length;
    ...
    return(
        <Slider>
            <h1 style={{fontSize:25,margin:10}}>Popular</h1>
            <AnimatePresence initial={false} onExitComplete={toggleLeaving}>
                <Row
                    ...
                    key={index}
                >
                {dataPopular?.results.slice(offset*index,offset*index+offset).map(tv=>
                                    <Box 
